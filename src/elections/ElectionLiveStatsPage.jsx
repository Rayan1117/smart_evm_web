...
export default function ElectionLiveStatsPage() {
  ...
  useEffect(() => {
    ...
    const init = async () => {
      try {
        socket = io("https://voting-api-wnlq.onrender.com/live-election", {
        ...
        const res = await fetch(
          `https://voting-api-wnlq.onrender.com/utils/get-vote-count/${electionId}`,
          {
        ...
        if (!data[0]) throw new Error("No election data found");
  ...
  if (loading) {
    return (
      <div style={{ minHeight: '100vh', background: theme.bg, display: 'flex', alignItems: 'center', justifyContent: 'center', fontFamily: theme.fontFamily }}>
        <div style={{ background: theme.cardBg, padding: '2rem 3rem', borderRadius: '1rem', boxShadow: theme.boxShadow, fontWeight: 600 }}>
          Loading live election stats…
        </div>
      </div>
    );
  }
  ...
  if (error) {
    return (
      <div style={{ minHeight: '100vh', background: theme.bg, display: 'flex', alignItems: 'center', justifyContent: 'center', fontFamily: theme.fontFamily }}>
        <div style={{ background: theme.errorBg, color: theme.errorText, padding: '1.5rem 2rem', borderRadius: '1rem', boxShadow: theme.boxShadow, maxWidth: '28rem', textAlign: 'center' }}>
          {error}
        </div>
      </div>
    );
  ...
  return (
    <div style={{ minHeight: '100vh', backgroundColor: theme.bg, fontFamily: theme.fontFamily, padding: '2rem' }}>
      <div style={{ maxWidth: '5xl', margin: '0 auto', background: theme.cardBg, borderRadius: '1.5rem', boxShadow: theme.boxShadow, padding: '2rem', display: 'flex', flexDirection: 'column', gap: '2.5rem' }}>
        <h1 style={{ fontSize: '2.25rem', fontWeight: 700, textAlign: 'center', color: theme.textPrimary }}>
          📊 Election Live Stats
        </h1>

        <div>
          <h2 style={{ fontSize: '1.5rem', fontWeight: 600, marginBottom: '1rem', color: theme.textPrimary }}>
            🧑‍💼 Candidates
          </h2>
        ...
        <div>
          ...
          <p style={{ fontSize: '1.25rem', fontWeight: 700, color: theme.textPrimary }}>
            {totalVotes}
          </p>
        </div>
